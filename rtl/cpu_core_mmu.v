// =====================================================================
// cpu_core_mmu.v  -  Single-cycle RV32IM + Zicsr + privilege + Sv32 MMU
...
module cpu_core_mmu #(
    ...
    // ---- page-table-walk read ports (to RAM, for the MMU) ----
    output wire [31:0] walk_addr1,
    input  wire [31:0] walk_data1,
    output wire [31:0] walk_addr2,
    ...
    // ---- Sv32 MMU: translate the data address (user mode only) -----
    wire [31:0] satp_val;
    wire [31:0] phys_addr;
    wire        xlate_active, data_pf, data_pf_store;
    wire        is_load_acc  = mem_read;
    wire        is_store_acc = mem_write;
    mmu u_mmu (
        .satp(satp_val), .priv(cur_priv), .va(alu_result),
        .is_load(is_load_acc), .is_store(is_store_acc),
        .req(mem_read | mem_write),
        .walk_addr1(walk_addr1), .walk_data1(walk_data1),
        .walk_addr2(walk_addr2), .walk_data2(walk_data2),
        .pa(phys_addr), .active(xlate_active),
        .fault(data_pf), .fault_store(data_pf_store)
    );
    ...
    assign dmem_addr   = phys_addr;      // translated (PA); = VA when xlate off
